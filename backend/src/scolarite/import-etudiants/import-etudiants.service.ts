...
export class ImportEtudiantsService {
  ...
  async renvoyerAcces(
    ...
    if (!etudiant.utilisateur) {
      throw new NotFoundException(
        'Aucun compte utilisateur lié à cet étudiant',
      );
    }

    if (!etudiant.email) {
      throw new BadRequestException(
        'Aucune adresse e-mail enregistrée pour cet étudiant',
      );
    }
    ...
    const ancienDoitChanger =
      etudiant.utilisateur
        .doitChangerMotDePasse;

    const ancienneDateChangement =
      etudiant.utilisateur
      ...
      await this.prisma
        ...
        .update({
          ...
          data: {
            ...
            dateChangementMotDePasse:
              ancienneDateChangement,
          },
        });

      throw new ServiceUnavailableException(
        'ECHEC_ENVOI_EMAIL_ACCES',
      );
    }
